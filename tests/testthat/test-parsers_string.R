...
test_that("'by_split' and 'by_symbol' trackers stay on current line", {
  ...
  set_LNR(5)
  expect_identical(tracker_val(by_split(exactly(3,literal("a")), "\\t")("a\ta\ta\ta")),5L)
  expect_identical(tracker_val(by_symbol(exactly(3,literal("a"))) ("aa")),5L)
  set_LNR(2)
  expect_identical(tracker_val((literal("A") %then% by_symbol(exactly(3,literal("a"))))(c("A","aa"))), 3L)
})
