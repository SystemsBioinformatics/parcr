...
A fasta file could look as follows
...
It consists of one or more sequence blocks, each with a header[^1] and a 
sequence, and each sequence consists of one or more lines of consecutive symbols
without spaces, representing the nucleotides. A sequence block could be preceded
by one or more empty lines, but doesn't have to be. This description already 
allows us to create a few elements of the `fasta` parser that we want to make:
...
extract the individual sequences and headers from it. We can structure this 
...
We know that the first element in each of these list elements is the header, and 
